...
def extract_embeddings(
    ...
    setup_logger(f"data/vq_{params.model_name}/log/log-{params.model_name}-cb-indexes")
    if params.num_jobs > 1:
        manifest = manifest[rank]
        output_manifest = params.embedding_dir / f"{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}-{rank}.jsonl.gz"
        embedding_path = params.embedding_dir / f'{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}-{rank}'
    else:
        output_manifest = params.embedding_dir / f"{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}.jsonl.gz"
        embedding_path =  params.embedding_dir / f'{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}'
    ...
    model.to(device)
    model.eval()
    logging.info(f"Number of model params: {sum(p.numel() for p in model.parameters())}")
    ...
    with NumpyHdf5Writer(embedding_path) as writer:
        logging.info(f"Writing {params.model_name} indexes to {embedding_path}")
        for i, batch in enumerate(dl):
            cuts = batch["cuts"]            
            embeddings, embedding_lens = model.extract_features(
                batch, 
    ...
    logging.info(f"Finished extracting codebook indexes, processed a total of {num_cuts} cuts.")
...
if __name__=="__main__":
    ...
    embedding_manifest = params.embedding_dir / f"{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}.jsonl.gz"
    ...
    if not embedding_manifest.exists():
        if nj == 1:
            ...
            manifests =  f"{str(params.embedding_dir)}/{params.model_name}-{params.model_version}-layer-{params.embedding_layer}-{params.manifest_name}-*.jsonl.gz"
